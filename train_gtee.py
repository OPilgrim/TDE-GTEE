...
from src.base_gtee.OneIE_data_module import OneIEDataModule
from src.base_gtee.model import GenIEModel
...
def main():
    ...
    # Required parameters
    parser.add_argument( "--model", default='constrained-gen', type=str, choices=['gen', 'constrained-gen'])
    ...
    parser.add_argument( '--test_file', default='data/ACE05-E+/test.oneie.json', type=str)
    parser.add_argument( '--inference', default=False, action="store_true", help="Whether or not to inference.")
    parser.add_argument( '--az_file', default=None, type=str, help="If you already know what type of event this is.")
    ...
    parser.add_argument( "--train_batch_size", default=24, type=int, help="Batch size per GPU/CPU for training.")
    parser.add_argument( "--eval_batch_size", default=48, type=int, help="Batch size per GPU/CPU for evaluation.")
    ...
    parser.add_argument( "--fp16", action="store_true", default=False, help="Whether to use 16-bit (mixed) precision (through NVIDIA apex) instead of 32-bit",)
    parser.add_argument( '--sample-gen', action='store_true', default=False, help='Do sampling when generation.')
    
    parser.add_argument( '--backbone_model', type=str, default='facebook/bart-large', choices=['facebook/bart-large'])
    ...
    parser.add_argument( "--cased", default=False, action="store_true", help="True means need lower.")
    parser.add_argument( "--debug", default=False, action="store_true", help="")
    parser.add_argument( "--output_attentions", default=False, action="store_true", help="")
    ...
    tb_logger = TensorBoardLogger(os.path.join(f'{args.ckpt_dir}/logs'))

    model = GenIEModel(args)
    dm = OneIEDataModule(args)
